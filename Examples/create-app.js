...
function linkCodePush(androidStagingDeploymentKey, iosStagingDeploymentKey) {
  ...
  if (isReactNativeVersionLowerThan(60)) {
    nexpect
      .spawn(`react-native link react-native-code-push`)
...
function optimizeToTestInDebugMode() {
  ...
  execCommand(
    `perl -i -p0e 's/#ifdef DEBUG.*?#endif/jsCodeLocation = [CodePush bundleURL];/s' ios/${appName}/${getAppDelegateName()}`,
  )
  reactNativeVersionIsLowerThanV073 &&
...
function isReactNativeVersionLowerThan(version) {
  if (!reactNativeVersion || reactNativeVersion == 'react-native@latest' || reactNativeVersion == 'react-native@next')
    return false
...
function androidSetup() {
  ...
  const mainApplicationPath = path.join(
    ...
    'com',
    appName,
    `MainApplication.${mainApplicationType}`,
  )
  ...
  const codePushGradleLink = `\napply from: "../../node_modules/@mathpresso/react-native-code-push/android/codepush.gradle"`
  if (reactGradleLink != null) {
    buildGradleContents = buildGradleContents.replace(reactGradleLink[0], `${reactGradleLink[0]}${codePushGradleLink}`)
  ...
  const codePushProjectImport = `':react-native-code-push'\nproject(':react-native-code-push').projectDir = new File(rootProject.projectDir, '../node_modules/@mathpresso/react-native-code-push/android/app')`
...
function getAppDelegateName() {
  return fs.readdirSync(path.join('ios', appName)).find(file => file === 'AppDelegate.mm' || file === 'AppDelegate.m')
}
