...
class RNAndroid extends Platform.Android implements RNPlatform {
  ...
  buildFunction(androidDirectory: string): Q.Promise<void> {
    ...
    return TestUtil.getProcessOutput(`${gradlewCommand} clean`, { noLogStdOut: true, cwd: androidDirectory })
      .then(() =>
        TestUtil.getProcessOutput(`${gradlewCommand} assembleRelease --daemon`, {
          noLogStdOut: true,
          cwd: androidDirectory,
        }),
      )
    ...
    return this.getEmulatorManager()
      ...
      .then((targetEmulator: string) => {
        return TestUtil.getProcessOutput(
          ...
          { cwd: iOSProject, timeout: 30 * 60 * 1000, maxBuffer: 1024 * 1024 * 5000, noLogStdOut: true },
        )
      })
      .then<void>(
        () => {
...
class RNProjectManager extends ProjectManager {
  ...
  public setupProject(
    ...
    return TestUtil.getProcessOutput('npx react-native init ' + appName + ' --version 0.71.3 --install-pods', {
